...
stocks_close = pd.DataFrame(web.DataReader(['^MERV'], 'yahoo', fechamin, fechamax)['Close'])
...
fig1.add_trace(go.Bar(x=df['fecha'], y=df['puntaje'],
...
fig1.add_trace(
    go.Scatter(
        ...
        marker=dict(size=10,color ="mediumblue", colorscale='mint') # stocks_close["^MERV"]
        ),
    secondary_y=True,
    
    )
fig1.update_layout(
yaxis2=dict(title="Precio MERVAL",side="right")
)

fig1.update_layout(height=800, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
